...
A = 0.1
f_matrix=0:0.1:2; %non-dimensional forcing frequency
omega_mat=2*pi*f_matrix; %circular frequency
...
for omega = omega_mat
    for j=1:num_vor
     ...
     p = (omega/(2*pi))*ones(1, length(tf(1:end)));
     omega/(2*pi)
     plot(p, 1./tf(1:end), "o","MarkerSize",5)
     hold on
     clear p
end
hold off
