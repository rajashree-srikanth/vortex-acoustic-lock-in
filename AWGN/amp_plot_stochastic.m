...
A_mat=0:0.1:1.1; % non-dimensional forcing amplitude
...
dt=10^-5; %time step
num_vor=1000; %number of vortices shed 
...
for A = A_mat
    for j=1:num_vor
     ...
     p = A*ones(1, length(tf(800:end)));
     A
     plot(p, tf(800:end), "o",'color','#7E2F8E',"MarkerSize",5)
     hold on
     clear p
end
